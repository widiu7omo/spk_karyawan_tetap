...
function check_matching(){
	$db = new Database();
	$q = "SELECT count(*) as karyawan_left FROM karyawan LEFT OUTER JOIN data_kriteria ON karyawan.id = data_kriteria.karyawan_id WHERE data_kriteria.karyawan_id IS NULL";
	if($db->query($q)){
		$result = $db->fetch();
